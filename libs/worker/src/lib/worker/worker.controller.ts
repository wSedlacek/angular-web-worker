...
export class WorkerController<T> {
   ...
   * @param request request recieved from the `WorkerClient`
   */
  handleAccessible(request: WorkerRequestEvent<WorkerEvents.Accessible>) {
    let response: WorkerResponseEvent<any>;
    try {
      const metaData = WorkerUtils.getAnnotation<AccessibleMetaData[]>(
        ...
        'accessibles',
        []
      ).filter((x) => x.name === request.propertyName)[0];
  ...
  createSubscription(request: WorkerRequestEvent<WorkerEvents.Observable>): void {
    ...
    this.subscriptions[request.body.subscriptionKey] = (<Subject<any>>(
      this.worker[request.propertyName]
    )).subscribe(
      (val) => {
