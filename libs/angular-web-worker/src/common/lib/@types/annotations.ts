...
/**
 * Metadata attached to a worker's prototype for any properties decorated with `@Accessible()`. Contains details that describes how a `WorkerClient` can access the property
 */
export interface AccessibleMetaData {
  ...
  type: Function;
  /**
   * Determines whether the decorated worker property can be retrieved by a `WorkerClient`. Set as an optional parameter in the `@Accessible()` decorator
   * @defaultvalue true
   */
  get: boolean;
  /**
   * Determines whether the decorated worker property can be set from a `WorkerClient`. Set as an optional parameter in the `@Accessible()` decorator
   * @defaultvalue true
   */
  set: boolean;
  /**
   * Whether the decorated property's prototype is transferred after it has been serialized and deserialized during communication between a worker and a client. Set as an optional parameter in the `@Accessible()` decorator
   * @defaultvalue false
...
export interface CallableMetaData {
  ...
  returnType: Function;
  /**
   * Whether the returned value's prototype is transferred after it has been serialized and deserialized when it is brought back to a client. Set as an optional parameter in the `@Callable()` decorator
   * @defaultvalue false
 ...
 * Contains details that allows the argument's prototype to be transferred after it has been serialized and deserialized when sent from a client to be passed as an argument of a worker function.
 * @Experimental has limitations
 */
export interface ShallowTransferParamMetaData {
  ...
  type: Function;
  /**
   * Index of the argument in the functions call signature
   */
  argIndex: number;
}
