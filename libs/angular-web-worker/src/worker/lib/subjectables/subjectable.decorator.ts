 ...
 */
export const Subjectable = () => <T extends Object, Tkey extends keyof SubjectsOnly<T> & string>(
  target: T,
  propertyKey: Tkey
) => {
  const annotation: SubjectableMetaData = {
    name: propertyKey,
    type: Reflect.getMetadata('design:type', target, propertyKey),
  };

  WorkerUtils.pushAnnotation(target.constructor, WorkerAnnotations.Subjectables, annotation);
};
